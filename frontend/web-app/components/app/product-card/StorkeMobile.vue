  ...
  <div class="product-card-stroke">
    ...
    <div class="product-card-stroke__control">
      <div class="product-card-stroke__text">
        <p class="product-card-stroke__title" style="max-width: 10rem">
          {{ title }}
        </p>
        ...
        >
          <p class="product-card-stroke__price">
            {{
              discount
          ...
          <p class="product-card-stroke__discount" v-if="discount">
            {{ usePriceFormat(price) }}
          </p>
        </div>
      </div>
      ...
      <div style="display: flex; justify-content: space-between">
        <div>
          ...
          <div class="col self-center">
            <svg
              ...
              @click="() => {
                if (favorites.find(_id) === -1) {
                  return favorites.add(_id)
                }
            ...
            <svg
              style="width: 1.3rem; height: 1.5rem; margin-left: 2.5rem"
              viewBox="0 0 15 17"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              @click="() => {cart.delAll(_id, sizeId)}"
            >
              <path
        ...
        <ClientOnly>
          <div class="counter">
            ...
            <q-btn flat round class="counter__btn" @click="() => {cart.add({product_id: _id, size_id: sizeId as string})}">+</q-btn>
          </div>
        </ClientOnly>
      </div>
    </div>
...
.product-card-stroke {
  display: grid;
  min-height: 15.4rem;
  padding: 2.3rem 2.6rem!important;
