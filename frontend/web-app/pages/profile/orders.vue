...
<template>
  <template v-if="!isMobile">
    <app-container-info style="margin-bottom: 2rem;" prefix="заказов" :quantity="count">
      <span style="font-weight: 500; font-size: 2.5rem">Мои заказы</span>
    </app-container-info>
    <template v-for="order in orders" :key="order._id">
      <app-card-order-big
        style="margin-bottom: 1.2rem;"
        v-if="!['refaund', 'completed_refaund'].includes(order?.status)"
        :status="order.status"
        :trakNumber="order.delivery_details?.tracking_code"
        :orderId="order._id"
